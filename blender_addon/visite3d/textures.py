...
class VISITE3D_OT_ResizeAllTextures(bpy.types.Operator):
    ...
    max_size: bpy.props.EnumProperty(
        name="Max Size", items=RESIZE_PRESETS, default="1024",
    )

    def execute(self, context):
        images = get_all_images()
        mem_before = calc_memory(images)
        count = sum(1 for img in images if resize_image(img, int(self.max_size)))
        mem_after = calc_memory(images)
        self.report({'INFO'},
            f"{count}/{len(images)} resized to max {self.max_size}px — "
...
class VISITE3D_OT_ResizeSelectedTextures(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        mem_after = calc_memory(images)
        self.report({'INFO'},
            f"{count}/{len(images)} resized — {mem_before:.1f} MB -> {mem_after:.1f} MB")
        return {'FINISHED'}
...
class VISITE3D_OT_TextureStats(bpy.types.Operator):
    ...
    bl_description = "Show texture count, sizes, and estimated VRAM"
...
class VISITE3D_OT_PackAndConvert(bpy.types.Operator):
    ...
    bl_description = "Convert non-alpha textures to JPEG (smaller GLB export)"
    bl_options = {'REGISTER', 'UNDO'}

    quality: bpy.props.IntProperty(name="Quality", default=85, min=10, max=100)
        ...
        for img in images:
            if img.channels == 4 and img.alpha_mode != 'NONE':
                ...
                if img.size[0] * img.size[1] <= 4096 * 4096:
                    ...
                    for i in range(3, len(pixels), 4 * 64):
                        if pixels[i] < 0.99:
                            has_alpha = True
                            break
                if has_alpha:
...
class VISITE3D_PT_TexturePanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.operator("visite3d.texture_stats", icon='INFO')
        row.operator("visite3d.list_large_textures", icon='ZOOM_ALL')
